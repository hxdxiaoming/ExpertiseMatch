...
def get_any_biencoder_component_for_infer(cfg, use_instruct=True, **kwargs):
	...
	# 使用本地模型路径
	if cfg["pretrained_model_cfg"] in ['microsoft/BiomedNLP-PubMedBERT-base-uncased-abstract', 'microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract']:
		cfg["pretrained_model_cfg"] = './model/BiomedNLP-BiomedBERT-base-uncased-abstract'
...
if dataset == 'SciRepEval' or dataset == 'SIGIR':
	if factor == 'topic':
