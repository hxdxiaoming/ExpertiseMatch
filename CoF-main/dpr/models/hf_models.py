...
model_mapping = {
    ...
    './model/BiomedNLP': (BertConfig, BertTokenizerFast, BertModel),
...
moe_model_mapping = {
    ...
    './model/BiomedNLP': (BertConfig, BertTokenizerFast, (MoEBertModel, BertModel)),
...
instruct_model_mapping = {
    ...
    './model/BiomedNLP': (BertConfig, BertTokenizerFast, (InstructBertModel, BertModel)),
}
...
class HFEncoder(nn.Module):
    def __init__(self,
        ...
        if "sentence-transformer" in cfg_name:
            ...
            # 处理本地路径的情况
            if os.path.isabs(cfg_name) or cfg_name.startswith('./'):
                # 对于本地路径，检查是否包含BiomedNLP
                if 'BiomedNLP' in cfg_name:
                    model_name = './model/BiomedNLP'
                else:
